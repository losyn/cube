...
function _.map(list, func)
  ...
  _.each(list, function(value, key, original_list)
    table.insert(newList, func(value, key, original_list))
  end)
...
function _.any(list, func)
  ...
  _.each(list, function(value, index, object)
    if func(value, index, object) then
      return true
    end
  end)
  return false
...
function _.split(value, pattern)
  ...
  local result = {}
  for match in (value .. pattern):gmatch("(.-)" .. pattern) do
...
_.fileName = function(fName)
  if type(fName) ~= "string" then
    return nil;
  end
  local idx = fName:match(".+()%.%w+$");
  return idx and fName:sub(1, idx - 1) or fName;
end

_.fileExtension = function(fName)
  return type(fName) ~= "string" and nil or fName:match(".+%.(%w+)$");
end
